...
impl Frog {
    ...
    pub fn hop(&mut self) {
        ...
        info!(target: "Frog.hop()", "A frog hopped and has {} energy remaining", self.energy);
        if self.energy == 0 {
            // 4. Use warn!() to warn that the frog will go to sleep since he ran out of energy
            warn!(target: "Frog.hop()", "Frog is now going to sleep; ran out of energy");
            self.sleep();
        }
    }
...
impl Default for Frog {
    fn default() -> Self {
        ...
        trace!(target: "Frog::default()", "Default value was generated, at a debug level");
        Frog {
            energy: 5,
            sleeping: false,
        }
